...
type basicCollector struct {
	...
	// A parameterized funcition used to gather metrics
	stats func() ([]NginxStats, error)
}

// NewBasicCollector construcs a collector using stats function.
...
// Discribe implements prometheus.Collector.
func (c *basicCollector) Describe(ch chan<- *prometheus.Desc) {
	// Gether metadata about each metrics
	ds := []*prometheus.Desc{
		c.ConnectionsActive,
	}
...
func (c *basicCollector) Collect(ch chan<- prometheus.Metric) {
	// Take a stats snapshot. Mush be concurrentcy safe.
	stats, err := c.stats()

	if err != nil {
		// if an error occurs, send an invalid metics to notify
